...
void setup() {
  ...
  if (buttonState == HIGH )//|| displayError != 0) // TODO
  {
...
void loop() {
  ...
  if (transmitConfirmedData)
  {
    ...
    while (!dataBuffer.isEmpty())
    {
      ...
      if(batteriedata) batteriedata->writeDataBatterie(dataset);
  ...
  Serial.write("BatterieData Volume ");
  Serial.print(batteriedata->getDataLength());
  if ((batteriedata->getDataLength() > 1000) || buttonstate_l) {
	    batteriedata->writeDataToSD();
  }
